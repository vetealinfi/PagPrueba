...
(function ($) {
	$.fn.wSwitcher = function (options) {
		...
		return this.each(function () {
			...
			if ($.cookie('astra_switcherIsOpen') == 1) {
				switcher.css('left', '0');
			}
			...
			control.click(function(){
				if (switcher.css('left') == '0' || switcher.css('left') == '0px'){
					...
					$.cookie('astra_switcherIsOpen', 0, {expires: 1, path: '/'});
				} else {
					switcher.animate({ left: '0'}, 250);
					$.cookie('astra_switcherIsOpen', 1, {expires: 1, path: '/'});
				}

			});

			if (window.body_layout == 'Wide') {
				switcher.find('.layout_type_wide').addClass('active');
			} else {
				switcher.find('.layout_type_boxed').addClass('active');
			}

			if ($.cookie('astra_switcherLayout') == 'type_wide') {
			...
			if ($.cookie('astra_switcherLayout') == 'type_boxed') {
				...
				$(this).click(function(){
					if ( ! $(this).hasClass('active')){
						...
						if (! canvas.hasClass(layout)) {
							...
							$.cookie('astra_switcherLayout', layout, {expires: 1, path: '/'});
			...
			if ($.cookie('astra_switcherColors') != undefined) {
				switcher.find('.w-switcher-section.for_color .w-switcher-section-item.active').removeClass('active');
				$('.w-switcher-section.for_color .w-switcher-section-item.color_'+$.cookie('astra_switcherColors')).addClass('active');

				$('.w-logo-img').attr('src', window.wp_template_dir_uri+'logo_'+$.cookie('astra_switcherColors')+'.png');

				$('#us_colors_inline').remove();

				link.attr('href', window.wp_template_dir_uri+'colors/color_'+$.cookie('astra_switcherColors')+'.css');
			...
			switcher.find('.w-switcher-section.for_color .w-switcher-section-item').each(function(){
				$(this).click(function(){
					if ( ! $(this).hasClass('active')){
						...
						$.cookie('astra_switcherColors', color, {expires: 1, path: '/'});
					}
				});
			});
